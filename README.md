...
The selected compiler settings are maintained directly in this file so their
changes can be reviewed here. Inspect the resolved settings, including options
implied by other settings, with:
...
`verbatimModuleSyntax` also enables `isolatedModules` checks, and `NodeNext`
implies `esModuleInterop`. Their behavior remains active without extra entries.

### Source maps and Node stack traces

| Setting               | What it does                                                                         | Why it is here                                                                   | Example                                                                         |
| --------------------- | ------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| `sourceMap: true`     | Emits a separate map linking generated JavaScript locations to TypeScript locations. | Helps debuggers and error-reporting tools point to the original code.            | Building `src/example.ts` produces `dist/example.js` and `dist/example.js.map`. |
| `inlineSources: true` | Embeds the original TypeScript text inside the map.                                  | Lets compatible tools display source even when only compiled output is deployed. | `example.js.map` includes the TypeScript text in `sourcesContent`.              |

Generating maps and enabling Node to use them are separate steps. Include
`--enable-source-maps` in the consuming application's start command. For example,
if its compiled entry point is `dist/server.js`:

```sh
node --enable-source-maps dist/server.js
```

Deploy the matching `.js` and `.js.map` files together. The map translates stack
locations; embedded source lets tools show the code without a separate `src/`
directory. Maps contain the original source and add to build size. Node also
does additional work when mapping error stacks. `pnpm typecheck` uses `--noEmit`,
so it produces neither JavaScript nor maps.

See the [source-map option](https://www.typescriptlang.org/tsconfig/sourceMap.html),
[embedded-source option](https://www.typescriptlang.org/tsconfig/inlineSources.html),
and [Node runtime flag](https://nodejs.org/api/cli.html#--enable-source-maps).
